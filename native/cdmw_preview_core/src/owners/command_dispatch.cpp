int run_cli(int argc, char** argv) {
    ...
    try {
        ...
        if (argc >= 4 && std::string(argv[1]) == "preview-job") {
            return run_preview_job(utf8_path(argv[2]), utf8_path(argv[3]));
        }
        if (argc >= 4 && std::string(argv[1]) == "mesh-audit-job") {
            return run_mesh_audit_job(utf8_path(argv[2]), utf8_path(argv[3]), argc >= 5 ? std::string(argv[4]) : std::string());
        }
        if (argc >= 4 && std::string(argv[1]) == "mesh-parse-job") {
            return run_mesh_parse_job(utf8_path(argv[2]), utf8_path(argv[3]), argc >= 5 ? std::string(argv[4]) : std::string());
        }
        if (argc >= 5 && std::string(argv[1]) == "mesh-rebuild-job") {
            return run_mesh_rebuild_job(utf8_path(argv[2]), utf8_path(argv[3]), utf8_path(argv[4]));
        }
...
int run_cli_utf16(int argc, wchar_t** argv) {
    ...
    for (int i = 0; i < argc; ++i) arguments.push_back(cdmw_native_diag::wide_to_utf8_diag(argv[i]));
