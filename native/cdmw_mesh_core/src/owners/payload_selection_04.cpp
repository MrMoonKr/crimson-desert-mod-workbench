...
std::set<int> mesh_editor_apply_face_selection_edit(
    const MeshSessionSubmesh& submesh,
    const MeshEditorSelection& incoming,
    int submesh_index,
    const std::string& operation,
    bool all_operation,
    bool invert_operation,
    int iterations
) {
    if (all_operation) {
...
std::set<std::array<int, 2>> mesh_editor_apply_edge_selection_edit(
    const MeshSessionSubmesh& submesh,
    const MeshEditorSelection& incoming,
    int submesh_index,
    const std::string& operation,
...
std::set<int> mesh_editor_apply_vertex_selection_edit(
    const MeshSessionSubmesh& submesh,
    const MeshEditorSelection& incoming,
    int submesh_index,
    const std::string& operation,
    ...
    const std::vector<std::set<int>> adjacency = build_vertex_adjacency(
        submesh.vertices.size(), submesh.faces
    );
    for (int iteration = 0; iteration < std::max(0, iterations); ++iteration) {
        if (operation == "grow") {
            ...
            for (const int vertex_index : selected) {
                ...
                if (!keep) {
                    keep = std::all_of(neighbors.begin(), neighbors.end(), [&](int neighbor) {
                        return selected.find(neighbor) != selected.end();
                    });
                }
                if (keep) {
            ...
            for (std::size_t vertex_index = 0; vertex_index < adjacency.size(); ++vertex_index) {
                ...
                const int selected_neighbors = static_cast<int>(std::count_if(
                    neighbors.begin(), neighbors.end(), [&](int neighbor) {
                        return selected.find(neighbor) != selected.end();
                    }
                ));
                const double ratio = static_cast<double>(selected_neighbors)
                    / static_cast<double>(std::max<std::size_t>(1, neighbors.size()));
...
MeshEditorSelection mesh_editor_apply_selection_edit(
    ...
    for (const int submesh_index : targets) {
        ...
        if (target_mode == "face") {
            std::set<int> selected = mesh_editor_apply_face_selection_edit(
                submesh,
                incoming,
                submesh_index,
                operation,
        ...
        if (target_mode == "edge") {
            std::set<std::array<int, 2>> selected = mesh_editor_apply_edge_selection_edit(
                submesh,
                incoming,
                submesh_index,
                operation,
        ...
        std::set<int> selected = mesh_editor_apply_vertex_selection_edit(
            submesh,
            incoming,
            submesh_index,
            operation,
