...
- `cdmw/ui/preview/` is the shared visible-model host. Its `preview` profile is read-only and its `authoring` profile enables Mesh Editor mutations; both use one verified resident process, monotonic package generations, latest-wins replacement, stale rejection, package leases, bounded shutdown, and visibility-scoped retry. Readiness remains fail-closed for helper provenance, protocol capabilities, the exact production backend, GPU ownership, and explicit renderer failure; documented material-audit limitations remain warnings and must not recycle a healthy renderer. A new-package view reset replaces both resident and host-local camera state with centered `1x` fit before render-tuning replay; Archive Browser selects straight-on character/general views and overhead weapon-family views, while same-package refresh preserves the live camera. Fit clears pan and fitted-relative zoom without changing the current angle. Python/C++ retain decode and mutation authority, never visible rendering. Authoring recovery replays the authoritative MeshService snapshot before input resumes; no surface falls back to a legacy renderer.
