...
- The 2026-07-17 expanded baseline covers 162 unique PACs (156 additions plus six repeat controls) at 136 PASS/24 CONCERN/2 FAIL and exposed shared metal-floor/tint and inferred-cutoff defects. Inferred PAC hair/cutout uses the established `0.12` cutoff only when no explicit cutoff exists. Fresh repair proof is 6/2/0 across eight alpha controls and 43/7/0 across 50 cross-category PACs; all weapons, swords, shields, helmets, upper armor, and hair controls pass. Every run retains depth, hidden resident ownership, integrity, and byte-identical archive fingerprints.
