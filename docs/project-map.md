...
| Archive browser and preview | `cdmw/ui/archive_browser/`; standalone v2 worker under `tools/dotnet_archive_backend/`; Python-free Lite product under `apps/Cdmw.ArchiveLite/`; shared semantic documents under `tools/dotnet_archive_backend/src/Cdmw.Archive.Content/`; native mmap/decode cores under `native/cdmw_full_archive_core/` and `native/cdmw_archive_core/` | `cdmw/domain/archives/` protocol/catalogue contracts; resident client/resource discovery in `cdmw/ui/shell/archive_backend_*.py`; `archive_catalogue_service.py`; remote paged model/controller/providers; focused `cdmw/core/archive_*.py` compatibility owners; shared capability manifest; native item catalog; retained legacy scan/cache workers for the transition release | `tests/test_archive_backend_*.py`, `tests/test_archive_catalogue_service.py`, `tests/test_archive_remote_*.py`, native self-tests, `Cdmw.FullArchive.Tests`, `apps/Cdmw.ArchiveLite/tests/`, `tests/test_archive_workflow_boundary.py`, `tests/test_release_packaging.py` | `tools/dotnet_archive_backend/README.md`, `apps/Cdmw.ArchiveLite/README.md`, `docs/features/archive-decoder-parity-and-lite-item-finder.md`, `docs/runbooks/worker-lifecycle.md`, `docs/features/archive-safety-model.md`, `docs/architecture.md` |
