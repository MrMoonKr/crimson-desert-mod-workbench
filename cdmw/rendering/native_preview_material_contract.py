...
from cdmw.rendering.native_preview_material_manifest import (
...
def _material_input_slot_state(slot_name: str, texture_input: PreviewMaterialTextureInput) -> Dict[str, object]:
    ...
    note_by_slot = {
        "clearcoat": "source clearcoat recorded; Rust Preview preview approximates it through specular response",
        "sheen": "source sheen recorded; Rust Preview preview approximates it through soft specular response",
        "transmission": "source transmission/volume recorded; Rust Preview preview does not render true glass",
        ...
        "unlit": "source unlit material recorded; Rust Preview preview uses flat non-PBR material hints",
    }
    return {
        "slot": slot_name,
        "preview_path": preview_path,
    ...
    *,
    ...
    if _batch_has_unlit_material_hint(batch):
        states["unlit"] = {
            ...
            "diagnostic": "source unlit material recorded; Rust Preview preview uses flat non-PBR material hints",
        }
    return states
...
def _material_contract_for_batch(
    ...
    *,
    ...
    if str(normalized_slot_states.get("transmission", {}).get("status", "") or "") != "missing":
        divergence_reasons.append("transmission/volume recorded but Rust Preview preview does not render true glass")
...
def _resolved_batch_material_category(
    ...
    *,
    ...
    nonmetal_tokens = _NONMETAL_DESCRIPTOR_TOKENS
