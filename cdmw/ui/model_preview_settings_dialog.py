...
class ModelPreviewSettingsDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        self.archive_renderer_backend_combo.addItem("Preview", self.ARCHIVE_RENDERER_D3D11)
        self.archive_renderer_backend_combo.setToolTip(
            "Preview is the only Archive Browser model-preview path."
        )
        ...
        self.d3d11_cull_back_faces_checkbox.setToolTip(
            "Draw only front-facing triangles in the Preview to inspect flipped winding or two-sided materials."
        )
        ...
        general_form.addRow("Preview view", self.d3d11_view_mode_combo)
        general_form.addRow("Preview normal Y", self.d3d11_normal_y_mode_combo)
        general_form.addRow("Preview texture address", self.d3d11_texture_address_mode_combo)
        ...
        self.d3d11_hint_label = QLabel(
            "Preview supports texture on/off, culling, view modes, Flip texture V, normal-Y override, sampler address mode, support-map shading, camera controls, zoom, fit, tool-side PBD physics preview, static HKX context when present, and exact DDS diagnostics."
        )
        ...
        self.quality_hint_label = QLabel(
            "Preview applies these to its shader and sampler directly. Texture resolution normally comes from exact DDS resources; generated fallback maps still use the existing preview cache pipeline."
        )
        ...
        self.preview_cache_limit_spin.setToolTip(
            "How many recently previewed entries the Archive Browser remembers, so revisiting one skips the rebuild. Each remembered entry is a small reference to its durable package, so this needs the Preview package cache below; with that set to Off, nothing can be remembered."
        )
        ...
        self.native_preview_cache_mode_combo.setToolTip(
            "Durable Preview package cache. Balanced keeps up to 512 MB of packages and 192 MB of decoded textures; Aggressive raises those to 2 GB and 512 MB. Off rebuilds every preview and clears what earlier modes wrote."
        )
        ...
        self.clear_preview_cache_button.setToolTip("Clears in-memory Archive Browser preview results, durable Preview packages, and the PAC XML profile index. Sidecar scan caches on disk are not removed.")
        preview_cache_layout.addRow("Remembered previews", self.preview_cache_limit_spin)
        preview_cache_layout.addRow("Preview package cache", self.native_preview_cache_mode_combo)
