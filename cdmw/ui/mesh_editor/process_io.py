...
_owned_exit_processes: dict[int, object] = {}


def unregister_owned_qprocess_for_exit(process: object) -> None:
    """Release a child from the interpreter-exit safety net after it stops."""

    _owned_exit_processes.pop(id(process), None)


def register_owned_qprocess_for_exit(process: object) -> None:
    """Keep an owned helper reachable so normal interpreter exit cannot strand it.

    The application-level kill-on-close job remains the crash/force-kill
    authority. This narrower fallback covers source tests and other normal
    Python exits that construct a Mesh Editor without running CDMW bootstrap.
    """

    _owned_exit_processes[id(process)] = process
    finished = getattr(process, "finished", None)
    connect = getattr(finished, "connect", None)
    if callable(connect):
        try:
            connect(lambda *_args, current=process: unregister_owned_qprocess_for_exit(current))
        except (RuntimeError, TypeError):
            pass


def _stop_owned_qprocesses_at_exit() -> None:
    processes = tuple(_owned_exit_processes.values())
    _owned_exit_processes.clear()
    for process in processes:
        ...
        try:
            ...
            process.waitForFinished(1000)  # type: ignore[attr-defined]
        except (AttributeError, RuntimeError):
            pass
        if process_id > 0 and qprocess_is_running(process):
            force_stop_windows_process_tree(process_id, include_root=True)


atexit.register(_stop_owned_qprocesses_at_exit)
