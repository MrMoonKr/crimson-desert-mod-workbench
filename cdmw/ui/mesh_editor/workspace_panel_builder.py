...
class WorkspacePanelBuilderMixin:
    ...
    def _build_rebuild_panel(self) -> QWidget:
        ...
        self.replace_from_archive_button = QToolButton(frame)
        self.replace_from_archive_button.setObjectName("MeshEditorReplaceFromArchiveButton")
        self.replace_from_archive_button.setText("Replace from Archive")
        self.replace_from_archive_button.setAccessibleName("Replace mesh from archive")
        self.replace_from_archive_button.setToolTip(
            "Build a separate loose mod that replaces this target with another mesh already in the loaded archives."
        )
        self.replace_from_archive_button.setProperty("meshEditorIconKey", "material_copy")
        self.replace_from_archive_button.setIcon(mesh_editor_action_icon("material_copy", self.palette()))
        self.replace_from_archive_button.setIconSize(QSize(18, 18))
        self.replace_from_archive_button.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        self.replace_from_archive_button.setEnabled(False)
        self.replace_from_archive_button.clicked.connect(self.replace_from_archive_requested.emit)
        self._ui_font_widgets.append(self.replace_from_archive_button)
        controls.addWidget(self.replace_from_archive_button)
