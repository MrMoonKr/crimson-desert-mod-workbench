...
from cdmw.ui.native_preview_protocol import (
    compact_nonnegative_indices as _compact_nonnegative_indices_impl,
    delete_after_paths as _delete_after_paths_impl,
    mesh_edit_json_groups as _mesh_edit_json_groups_impl,
    remove_paths as _remove_paths_impl,
    sorted_nonnegative_indices as _sorted_nonnegative_indices_impl,
    write_i32_preview_delta as _write_i32_preview_delta_impl,
)


def _write_i32_preview_delta(values: Sequence[int], suffix: str) -> tuple[dict[str, object], Path] | None:
    return _write_i32_preview_delta_impl(values, suffix)


def _sorted_nonnegative_indices(raw_values: Iterable[int] | None) -> list[int]:
    return _sorted_nonnegative_indices_impl(raw_values)


def _compact_nonnegative_indices(raw_values: Iterable[int] | None) -> tuple[tuple[int, int] | None, list[int]]:
    return _compact_nonnegative_indices_impl(raw_values)
...
def _mesh_edit_json_groups(groups: Sequence[Mapping[str, object]] | None) -> Sequence[Mapping[str, object]]:
    return _mesh_edit_json_groups_impl(groups)


def _delete_after_paths(value: object) -> tuple[Path, ...]:
    return _delete_after_paths_impl(value)
