...
def _app_shell_stylesheet(theme: Dict[str, str], metrics: Dict[str, int]) -> str:
    return f"""
    QWidget {{
        color: {theme["text"]};
    }}
...
def _app_section_stylesheet(theme: Dict[str, str], metrics: Dict[str, int]) -> str:
    return f"""
...
def _app_workflow_stylesheet(theme: Dict[str, str], role_text: Dict[str, str]) -> str:
    return f"""
...
def _app_controls_stylesheet(theme: Dict[str, str], metrics: Dict[str, int]) -> str:
    return f"""
...
def _app_feedback_stylesheet(theme: Dict[str, str]) -> str:
    return f"""
    QLabel#MetricChip {{
        color: {theme["text_strong"]};
        background: {theme["field_alt"]};
...
def build_app_stylesheet(theme_key: str, *, base_font_size: int = DEFAULT_UI_FONT_SIZE,
    ...
    return "".join((
        _app_shell_stylesheet(theme, metrics),
        _app_section_stylesheet(theme, metrics),
        _app_workflow_stylesheet(theme, role_text),
        _app_controls_stylesheet(theme, metrics),
