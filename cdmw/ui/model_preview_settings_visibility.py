...
_DOTNET_SETTING_EFFECTS = {
    ...
    "d3d11_wire_color": "sets the resident viewport wireframe color",
    "d3d11_vertex_color": "sets the resident viewport vertex marker color",
...
def sync_renderer_specific_controls(dialog: object) -> None:
    ...
    if dotnet:
        for tab_name, widgets in setting_widgets.items():
            ...
            for field, widget in widgets.items():
                ...
                if supported:
                    tooltip = (
                        f"Preview: {_DOTNET_SETTING_EFFECTS[field]}. "
    ...
    if dotnet:
        ...
        dialog.intro_label.setText(
            "Camera input and placement-Gizmo settings for the embedded Mesh Editor preview. Changes are sent live and saved with Preview Settings."
            if mesh_editor_dotnet
            else "Camera input settings for the resident Preview Archive Browser preview. Changes are sent live and saved with Preview Settings."
        )
        ...
        dialog.d3d11_hint_label.setText(
            "Archive Browser renderer settings are hidden while Preview owns the Mesh Editor preview."
        )
        ...
        dialog.controls_usage_hint_label.setText(
            "Preview camera controls: left-drag orbits; middle-drag, right-drag, or Shift+left-drag pans; the mouse wheel zooms; Fit resets framing. Each role pane keeps its own camera."
            if mesh_editor_dotnet
            else "Preview camera controls: left-drag orbits; middle-drag, right-drag, or Shift+left-drag pans; the mouse wheel zooms; Fit resets framing."
        )
    ...
    for widget, archive_label, embedded_label in (
        (dialog.d3d11_view_mode_combo, "Preview view", "Preview view"),
        (dialog.d3d11_normal_y_mode_combo, "Preview normal Y", "Preview normal Y"),
        (
            dialog.d3d11_texture_address_mode_combo,
            "Preview texture address",
            "Preview texture address",
        ),
    ):
    ...
    if not dotnet:
        dialog.high_quality_checkbox.setToolTip(
            "Preview packages and shades resolved normal/material/height support maps only when this is enabled."
        )
