    ...
    def _refresh_selected_part_enable_preview(source_indices: object) -> None:
        ...
        if callable(_state._alignment_d3d11_preview_active) and _state._alignment_d3d11_preview_active():
            ...
            if callable(set_status_message):
                set_status_message('Preview source enable preview commands are unavailable; preview is stale. Retry Preview to resync.', error=True)
            return
        if _state._selected_part_mesh_edit_active():
            set_status_message = getattr(_state.self.shell, 'set_status_message', None)
            if callable(set_status_message):
                set_status_message('Active Mesh Editor source enable preview requires Preview refresh; Python preview rebuild fallback is disabled.', error=True)
            return
