...
def _dotnet_preview_process_metrics(owner: object) -> Dict[str, object]:
    ...
    try:
        ...
        process_id = 0
    process_memory = _windows_process_memory_snapshot(process_id)
    if not process_memory:
        process_id = 0
    return {
        "dotnet_preview_process_pid": process_id,
        "dotnet_preview_process_private_bytes": _snapshot_value(process_memory, "private_bytes"),
        "dotnet_preview_process_working_set_bytes": _snapshot_value(process_memory, "working_set_bytes"),
        "dotnet_preview_process_generation": int(getattr(controller, "process_generation", 0) or 0),
        "dotnet_preview_package_generation": int(getattr(controller, "package_generation", 0) or 0),
        "dotnet_preview_process_running": bool(getattr(controller, "is_running", False)),
    }
...
class ArchivePreviewMemoryAuditMixin:
    ...
    def _archive_memory_audit_payload(
        ...
        dotnet_preview_metrics = _dotnet_preview_process_metrics(self)
        ...
        payload["memory_total_private_bytes"] = (
            ...
            + int(payload.get("dotnet_preview_process_private_bytes", 0) or 0)
        )
        return payload
    ...
    def _record_archive_memory_audit(
        ...
        *,
        ...
        dotnet_private = self._memory_mib(payload.get("dotnet_preview_process_private_bytes", 0))
        should_log = bool(log_if_high) and (
            main_private >= 1536.0 or preview_core_private >= 512.0 or dotnet_private >= 512.0
        )
        now = time.monotonic()
        if should_log and now - float(getattr(self, "archive_memory_audit_last_log_at", 0.0) or 0.0) >= 30.0:
            ...
            self.append_archive_log(
                ...
                f"dotnet_preview_private={dotnet_private:.1f} MiB; "
