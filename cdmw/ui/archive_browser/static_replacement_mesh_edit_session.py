...
def _mesh_editor_sync_static_replacement_session_to_working_mesh(_state, _callbacks, reason: str) -> bool:
    ...
    try:
        ...
        _state.self.shell.set_status_message(
            "Mesh Editor sync failed; reload the preview before continuing.",
            error=True,
        )
        return False
    _state._mesh_edit_state.replacement_mesh_for_mapping = mesh
...
def _mesh_edit_refresh_replacement_preview_model(_state, _callbacks,
    ...
    if (
        ...
        _state.self.shell.set_status_message(
            "Active Mesh Editor preview refresh requires Preview; Python preview rebuild fallback is disabled.",
            error=True,
        )
        return False
