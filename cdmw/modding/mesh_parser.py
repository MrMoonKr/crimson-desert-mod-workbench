...
def _decode_pac_normal(data: bytes, rec_off: int) -> tuple[float, float, float]:
    """Unpack the normal the way the game's own vertex shaders do.

    Only x and y are stored, in bits 10-19 and 20-29. The game reconstructs z on
    the unit sphere and takes its sign from bit 30, which is why nothing here
    reads bits 0-9: those carry the tangent's y component, and treating them as
    the normal's z is what used to return vectors of median length 1.13 rather
    than 1. The ``max`` guards the same case the shader's own clamp does, where
    10-bit quantization puts x^2 + y^2 a little above 1.
    """
    try:
    ...
    nx = ((packed >> 10) & 0x3FF) / 511.5 - 1.0
    ny = ((packed >> 20) & 0x3FF) / 511.5 - 1.0
...
def _decode_pac_extra_skin_slots(data: bytes, rec_off: int) -> tuple[int, ...]:
    ...
    """
    gate_offset = rec_off + PAC_SKIN_GATE_OFFSET
    if gate_offset >= len(data):
        return (-1,) * PAC_SKIN_EXTRA_INFLUENCES
    if (data[gate_offset] & PAC_SKIN_GATE_MASK) == PAC_SKIN_GATE_DISABLED:
        return (-1,) * PAC_SKIN_EXTRA_INFLUENCES
    try:
        halves = struct.unpack_from(f"<{PAC_SKIN_EXTRA_INFLUENCES}e", data, rec_off + PAC_SKIN_EXTRA_INDEX_OFFSET)
    except struct.error:
        return (-1,) * PAC_SKIN_EXTRA_INFLUENCES
    decoded: list[int] = []
    for value in halves:
        if not math.isfinite(value) or value < 0.0 or value > PAC_SKIN_MAX_BONE_INDEX:
            decoded.append(-1)
            continue
        decoded.append(int(math.floor(value + 0.5)))
...
def _decode_pac_vertex_records_bulk(
    ...
    *,
    ...
    # Same unpacking as _decode_pac_normal; see there for why bits 0-9 are not
    # the normal's z. tests/test_pac_vectorized_decode_equivalence.py holds the
    # two paths together.
    packed = u32[:, 4]
    nx = ((packed >> 10) & 0x3FF) / 511.5 - 1.0
    ny = ((packed >> 20) & 0x3FF) / 511.5 - 1.0
    nz = np.sqrt(np.maximum(0.0, 1.0 - nx * nx - ny * ny))
    nz = np.where(((packed >> 30) & 1).astype(bool), -nz, nz)
    normals = list(zip(nx.tolist(), ny.tolist(), nz.tolist()))
    ...
    if include_skin:
        ...
        if not live_mask.any():
            ...
            totals = np.where(live_mask, raw_weights, 0).sum(axis=1).astype(np.float64)
            scaled = raw_weights.astype(np.float64) / np.where(totals > 0.0, totals, 1.0)[:, None]
            empty = ()
            bone_indices = [empty] * n
            bone_weights = [empty] * n
...
def parse_pac(data: bytes, filename: str = "") -> ParsedMesh:
    ...
    candidates: list[tuple[int, int, int, int, ParsedMesh]] = []
    for geom_section_idx in [4, 3, 2, 1]:
        geom_sec = sec_by_idx.get(geom_section_idx)
        if geom_sec is None:
            continue
        lod = 4 - geom_section_idx
        try:
            candidate = _parse_pac_geometry_section(data, filename, descriptors, geom_sec, lod)
        except Exception as exc:
            logger.debug(
                "PAC %s: section %d (lod %d) recovery failed: %s",
                filename,
                geom_section_idx,
                lod,
                exc,
            ...
            continue
        if not candidate.submeshes or candidate.total_faces <= 0:
            continue
        candidates.append(
            (
                candidate.total_faces,
                candidate.total_vertices,
                len(candidate.submeshes),
                geom_section_idx,
                candidate,
            )
        )
    ...
    logger.info(
        "Parsed PAC %s: %d submeshes, %d verts, %d faces",
        filename,
        len(result.submeshes),
...
def _parse_pac_geometry_section(
    data: bytes,
    filename: str,
    descriptors: list[PacDescriptor],
    geom_sec: dict,
    ...
    result = ParsedMesh(path=filename, format="pac")
    if lod < 0:
        raise ValueError("PAC LOD index is invalid.")

    total_indices = sum(d.index_counts[lod] for d in descriptors if lod < len(d.index_counts))
    ...
    for desc in descriptors:
        vertex_count = desc.vertex_counts[lod] if lod < len(desc.vertex_counts) else 0
        desc_vert_offsets.append(vert_cursor)
        vert_cursor += vertex_count * 40

    for di, desc in enumerate(descriptors):
        vc = desc.vertex_counts[lod] if lod < len(desc.vertex_counts) else 0
        ic = desc.index_counts[lod] if lod < len(desc.index_counts) else 0
        if vc == 0 and ic == 0:
            continue
        ...
        if max_idx >= vc:
            partner_idx = next(
                (
                    pj
                    for pj, partner in enumerate(descriptors)
                    if pj != di and lod < len(partner.vertex_counts) and partner.vertex_counts[lod] > max_idx
                ),
                None,
            )
