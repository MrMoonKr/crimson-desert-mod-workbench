...
def render_report(manifest: Mapping[str, object]) -> str:
    ...
    out.append(
        ...
        "manifest entry and regenerate. This is a contributor research-progress heuristic, "
        "not a percentage of supported operations, editable assets or verified game behavior. "
        "The manifest remains a maintained claim that requires code and test evidence."
    )
    out.append("")
    out.append("## Contributor progress heuristic")
    out.append("")
    out.append(
    ...
    out.append(
        _table(
            ...
            ["Scope", "Formats", "Decode heuristic", "Write heuristic"],
        )
    )
    out.append("")
    out.append(
        "The heuristic uses assigned weights, not measured asset support: "
