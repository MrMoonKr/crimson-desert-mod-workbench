...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QScrollArea, QSplitter,
    ...
    QPlainTextEdit, QDialogButtonBox, QCheckBox, QComboBox, QSizePolicy)
...
def build_workspace(dialog, parts):
    ...
    right.addWidget(dialog._preview)
    ...
    dialog._review_view.setMaximumHeight(170)
    dialog._review_view.hide()
    ...
    actions.addWidget(dialog._preview_placement)
    ...
    actions.addWidget(dialog._cancel_preparation)
    dialog._show_files = QPushButton("Details and exact files")
    dialog._show_files.setCheckable(True)
    dialog._show_files.toggled.connect(dialog._review_view.setVisible)
    ...
    layout.addWidget(dialog._check_summary)
    layout.addWidget(dialog._blocker_label)
    layout.addWidget(dialog._review_view)
    layout.addWidget(dialog._buttons)
