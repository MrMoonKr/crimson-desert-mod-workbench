...
CDMW Mesh Editor ──JSONL + owned files + child HWND──>
...
command, state-snapshot, Finish, cancel, and error messages carry the protocol,
...
& Refit, and output-policy operations are typed asynchronous commands against
the shadow service. **Finish Edit Mesh** drains pending work and asks CDMW to
prepare the complete snapshot through its existing exact or Free Edit path.
Publication is one atomic authoritative transaction only while the opening mesh,
Geometry Layers, and Morph & Refit revisions/state still match. Rejection keeps
the Rust window and shadow alive; cancel, close, crash, or protocol failure
discards them. The Rust process never writes PAMT or PAZ archives directly.
...
remain lit and visible without turning the complete mesh into an X-Ray view.
The optional deformation overlay compares each current position with a
persistent loaded-topology reference, maps displacement magnitude from green
through yellow to red, and survives later gestures or a temporary UI toggle.
