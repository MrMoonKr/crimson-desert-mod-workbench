...
- No-window `LabApplication` construction plus 13 painted-control/input tests whose coordinates come from egui's clipped draw output: all fifteen preview modes, LOD menus, overlay and camera controls, texture relationship and material-parameter provenance, 24 selection domain/shape/depth combinations through the bounded raw-pointer route, topology-aware All/Grow/Shrink/Invert/Clear with exact selection history, all seven edit tools, three topology actions, exact Undo/Redo, disabled controls, 1×/1.5×/2× camera input, three resize shapes, Esc/resize cancellation, and dense face-selection fill without per-triangle outline strokes. Lower Inspector controls are verified at 1280×720 and 1000×600.
...
Thirteen no-window tests now construct the real `LabApplication`, discover control
...
selection-only Undo/Redo, and unchanged geometry. Mesh regressions separately
