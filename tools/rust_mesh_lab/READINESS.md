...
- Interactive Move, Rotate, Scale, Grab, Smooth, Inflate, and Pinch with Smooth/Linear/Constant visible-surface falloff, stable begin weights for Grab, resampled weights for the other brushes, and 1–8 Smooth passes; atomic face Delete/Subdivide/Duplicate/Extrude, selected-edge Subdivide, Duplicate as New Part, Undo, and Redo. Multi-face duplication retains patch adjacency through one isolated clone per source vertex; the New Part path assigns the next free submesh, preserves per-face material ownership, and lets the selected copy move without changing pre-existing vertices. Extrude uses a positive user-set distance, shares one offset cap clone per selected source vertex, adds walls only on selected-region boundaries, preserves every surviving source position and normal plus inherited owner/material/UV/normal attributes, removes newly unreferenced selected-region interior vertices, and selects only the cap. Topology failures leave the complete working state unchanged and generated caps/faces/edges become the deterministic selection.
...
- No-window `LabApplication` construction plus 15 painted-control/input tests whose coordinates come from egui's clipped draw output: all fifteen preview modes, LOD menus, overlay and camera controls, texture relationship and material-parameter provenance, 24 selection domain/shape/depth combinations through the bounded raw-pointer route, topology-aware All/Linked/Grow/Shrink/Invert/Clear with exact selection history and disconnected-island isolation, all seven edit tools, Smooth/Linear/Constant falloff and 1–8 Smooth passes, face Delete/Subdivide/Duplicate/Extrude, selected-edge Subdivide, Duplicate as New Part, exact Undo/Redo, disabled controls, 1×/1.5×/2× camera input, three resize shapes, Esc/resize cancellation, and dense face-selection fill without per-triangle outline strokes. The painted Extrude path consumes a non-default distance, requires every original synthetic-triangle vertex to remain exact, and validates the selected cap before Undo/Redo. Lower Inspector controls are verified at 1280×720 and 1000×600.
...
and extrusion to keep surviving source attributes exact while creating only a
connected cap plus boundary walls. These checks inspect application state and
...
|---|---:|---:|---:|
| 0 | 13,740 | 25,158 | 11/11 |
| 1 | 1,938 | 3,030 | 11/11 |
| 2 | 609 | 778 | 11/11 |
| 3 | 337 | 429 | 11/11 |

Move, Grab, Smooth, Inflate, Pinch, face Delete, face Subdivide, selected-edge
Subdivide, face Duplicate, Duplicate as New Part, and face Extrude each ran on
fresh working meshes for every LOD.
...
decode took 3.33 ms and all 44 complete operation/Undo/Redo scenarios took
789.80 ms. Single-face Extrude added exactly three vertices and six faces on
every LOD while preserving every original position and normal; operation samples
were 15.99, 2.00, 0.54, and 0.27 ms for LOD0 through LOD3 respectively. LOD3
