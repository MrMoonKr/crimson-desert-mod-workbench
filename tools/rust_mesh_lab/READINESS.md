...
- Direct3D 12 `wgpu` surface, depth target, persistent revisioned mesh/normal/bounds buffers, Textured/Base Color/Normal Map/UV Checker/Base Alpha/Material Response/Layer Mask/Solid/Solid+Wire/Wireframe/Vertices/Wire+Vertices/X-Ray modes, and independent Normals/Bounds overlays. Layer Mask is a grayscale inspection mode, not layer composition. Bones is visibly disabled until skeleton context exists.
...
Normal Map, UV Checker, Base Alpha, Material Response, Layer Mask, Solid Faces,
...
back material-role, metallic and dielectric Specular-map, metallic and dielectric Glossiness-map, opaque and cutout Opacity-map, positive and zero-scale Height-map, inactive non-hair and active hair Flow-map, layer-mask fallback/R/B-channel, emissive-factor,
roughness-factor, metalness-factor, metalness-plus-specular-factor, and fully
composed probes: 65 frames total. The D3D12 validation scope was
...
same post-frame boundary as the Windows runtime. They cover all thirteen preview modes and the LOD
