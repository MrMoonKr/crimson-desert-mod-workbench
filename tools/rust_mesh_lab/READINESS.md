...
- Interactive Move, Rotate, Scale, Grab, Smooth, Inflate, and Pinch with Smooth/Linear/Constant visible-surface falloff, stable begin weights for Grab, resampled weights for the other brushes, and 1–8 Smooth passes; atomic face Delete/Subdivide/Duplicate, selected-edge Subdivide, Duplicate as New Part, Undo, and Redo. Multi-face duplication retains patch adjacency through one isolated clone per source vertex; the New Part path assigns the next free submesh, preserves per-face material ownership, and lets the selected copy move without changing pre-existing vertices. Topology failures leave the complete working state unchanged and generated faces/edges become the deterministic selection.
...
|---|---:|---:|---:|
| 0 | 13,740 | 25,158 | 10/10 |
| 1 | 1,938 | 3,030 | 10/10 |
| 2 | 609 | 778 | 10/10 |
| 3 | 337 | 429 | 10/10 |

Move, Grab, Smooth, Inflate, Pinch, face Delete, face Subdivide, selected-edge
Subdivide, face Duplicate, and Duplicate as New Part each ran on fresh working
meshes for every LOD.
...
decode took 3.34 ms and all 40 complete operation/Undo/Redo scenarios took
698.17 ms. LOD3 contains three source edges shared by four faces;
