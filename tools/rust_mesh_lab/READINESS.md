...
- Offscreen D3D12 renderer coverage uploads nine DDS files with the live plan/upload helper, composes base/normal/packed-material/roughness/metalness/occlusion/emissive/specular roles on one range beside an independent base-colored range, applies separate explicit emissive, roughness, metalness, and metallic-specular factor probes, and exercises all seven preview modes with Normals and Bounds across 4:3, portrait, and widescreen targets. A validation error scope and CPU readbacks reject invalid or all-background output, require all eight sampled roles to change pixels independently, require Specular to change metallic pixels while leaving dielectric pixels identical, and require each factor class to change pixels again without constructing a window.
...
texture role and subresource, apply the remaining preserved scalar/vector factors and alpha policy, and compose layered materials
...
decoded fallback, resolve all eight sampled roles for one owner while reporting
...
and incomplete-value confidence. Loader fixtures require explicit emissive and roughness/metalness/specular factors to follow reordered LOD ownership and isolate conflicting values field by field. Renderer tests
...
offscreen D3D12 gate uploads nine synthetic DDS files, composes eight roles on
one material beside a second independent range, and compares unresolved,
base-only, one-extra-role, metallic and dielectric Specular-map, factored-emissive,
roughness-factor, metalness-factor, metalness-plus-specular-factor, and fully
composed CPU readbacks through the live helper during its 36-frame pass. Every
sampled role and each explicit factor class must change pixels independently;
Specular must change metallic pixels and leave dielectric pixels identical. This is synthetic relationship
...
back material-role, metallic and dielectric Specular-map, emissive-factor,
roughness-factor, metalness-factor, metalness-plus-specular-factor, and fully
composed probes: 36 frames total. The D3D12 validation scope was
