...
- Bounded material-sidecar scanning preserves wrapper type, submesh, shader/material name, texture parameter, original path, inferred role, and unknown texture references across common attribute spellings. Direct and archive loaders derive same-stem material sidecars, prefer explicit sidecar base-color references over decoded-name fallback, map wrapper owners independently onto every LOD's decoded submesh order, and use exact/relative/unambiguous asset relations without selecting ambiguous basenames. Disjoint owners may bind different DDS files; same-owner conflicts leave only that owner unresolved.
...
- Offscreen D3D12 renderer coverage uploads two DDS files with the live plan/upload helper, switches bind groups across two material ranges through the mesh draw dispatcher, and exercises all seven preview modes with Normals and Bounds across 4:3, portrait, and widescreen targets; a validation error scope and CPU readback reject invalid or all-background output without constructing a window.
...
| Native texture/material binding | Partial | Multiple sidecar base-color DDS files can render with role-correct color space on distinct owning submesh ranges, including reordered LOD ownership; arrays/cubes, reconstruction, packed/layer composition, other sampled roles, and fallback transcode remain incomplete |
...
sidecar texture ownership, resolves multiple disjoint base-color relationships,
applies role-correct sRGB/linear upload mapping, switches bindings per material
range and LOD, and keeps an explained placeholder for missing, ambiguous, or
conflicting owners. Lab readiness still requires the asset
...
and compose packed/layered material roles on their owning submeshes. The current
rainbow surface is the normal-based material approximation wherever no
...
decoded fallback, bind two distinct DDS files when their submesh owners are
disjoint, refuse two paths that claim the same owner, preserve owner identity
when lower-LOD submesh order changes, refuse a missing explicit DDS instead of
falling back, and retain one decoded reference only when no sidecar exists.
...
offscreen D3D12 gate uploads two synthetic DDS files and switches their bind
groups across two material ranges through the live helper before its 22-frame
validation/readback pass. This is synthetic relationship
