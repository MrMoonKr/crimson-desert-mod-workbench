...
pub fn run_headless_mesh_check(
    ...
    for (name, event) in scenarios {
        reports.push(run_scenario(document, &baseline_fingerprint, name, event)?);
    }
...
fn run_scenario(
    ...
    let (operation, operation_ms) = run_variant(document, std::slice::from_ref(&event))?;
    ensure!(
        operation.final_fingerprint != baseline_fingerprint,
        "headless {name} scenario made no geometry change"
    );
    let (undo, undo_ms) = run_variant(document, &[event.clone(), ReplayEvent::Undo])?;
    let (redo, redo_ms) = run_variant(document, &[event, ReplayEvent::Undo, ReplayEvent::Redo])?;
    let exact_undo_redo = undo.final_fingerprint == baseline_fingerprint
        && redo.final_fingerprint == operation.final_fingerprint;
    ensure!(
        exact_undo_redo,
        "headless {name} undo/redo mismatch: baseline={baseline_fingerprint}, operation={}, undo={}, redo={}",
        ...
        "headless {name} created the wrong history count"
    );
    ensure!(
        redo.history_entries == 1,
        "headless {name} redo did not restore history"
    );
...
fn run_variant(document: &MeshDocument, events: &[ReplayEvent]) -> Result<(ReplayReport, f64)> {
...
mod tests {
    ...
    #[test]
    fn synthetic_headless_mesh_check_covers_every_edit_family() -> Result<()> {
