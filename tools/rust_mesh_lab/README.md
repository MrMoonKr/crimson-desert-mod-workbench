...
- Choose Textured, Game Outdoor, Base Color, Normal Map, UV Checker, Base Alpha, Part ID, Material Response, Layer Mask, Solid Faces, Solid + Wire, Wireframe, Vertices, Wire + Vertices, or X-Ray from **Preview mode**, then independently toggle **Normals** and **Bounds**. Game Outdoor compares the same material under the production preview's approximate warm-direct/cool-ambient preset; it is not game-shader parity. Part ID gives every material-owner draw range a stable diagnostic color even when no texture is bound. Layer Mask shows `_colorBlendingMaskTexture` R or `_detailMaskTexture` B in grayscale and does not pretend to compose the selected dye/detail layers. **Bones** remains visibly disabled with its missing-skeleton reason until PAB/PAC binding is decoded.
- Choose Click, Brush, Rectangle, or Lasso, then drag in the viewport. Use **All Vertices**, **All Edges**, or **All Faces** for a complete domain, and **Grow**, **Shrink**, or **Invert** for topology-aware selection changes. Brush radius, Smooth/Inflate/Pinch strength, Smooth/Linear/Constant falloff, and 1–8 Smooth passes are adjustable in the inspector; Grab follows pointer travel directly and therefore has no separate strength control.
