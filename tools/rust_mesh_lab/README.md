...
- Choose Textured, Base Color, Normal Map, UV Checker, Base Alpha, Material Response, Layer Mask, Solid Faces, Solid + Wire, Wireframe, Vertices, Wire + Vertices, or X-Ray from **Preview mode**, then independently toggle **Normals** and **Bounds**. Layer Mask shows `_colorBlendingMaskTexture` R or `_detailMaskTexture` B in grayscale and does not pretend to compose the selected dye/detail layers. **Bones** remains visibly disabled with its missing-skeleton reason until PAB/PAC binding is decoded.
