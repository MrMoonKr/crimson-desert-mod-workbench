...
class NativePreviewCoreTests(unittest.TestCase):
    ...
    def test_native_preview_core_is_bundled_and_archive_worker_attempts_it(self) -> None:
        ...
        self.assertIn("capture_back_buffer_to_png", d3d11_text)
        ...
        self.assertIn("float d3d11_mip_lod_bias = -2.0f;", source_text)
        self.assertIn("float d3d11_light_azimuth_degrees = -10.0f;", source_text)
        self.assertIn("float d3d11_ao_strength = 0.45f;", source_text)
        self.assertIn("float d3d11_roughness_bias = -0.04f;", source_text)
        self.assertIn("float d3d11_metalness_scale = 1.45f;", source_text)
        self.assertIn("float d3d11_environment_strength = 0.62f;", source_text)
        self.assertIn("float d3d11_emissive_gain = 2.2f;", source_text)
        self.assertIn("float ambient_strength = 0.84f;", source_text)
        self.assertIn("float diffuse_wrap_bias = 0.58f;", source_text)
        self.assertIn("float specular_base = 0.055f;", source_text)
        self.assertIn("float specular_max = 0.52f;", source_text)
        self.assertIn("float shininess_max = 152.0f;", source_text)
        self.assertIn("float mip_lod_bias = -2.0f;", d3d11_text)
        self.assertIn("float light_azimuth_degrees = -10.0f;", d3d11_text)
        self.assertIn("float ao_strength = 0.45f;", d3d11_text)
        self.assertIn("float roughness_bias = -0.04f;", d3d11_text)
        self.assertIn("float metalness_scale = 1.45f;", d3d11_text)
        self.assertIn("float environment_strength = 0.62f;", d3d11_text)
        self.assertIn("float emissive_gain = 2.2f;", d3d11_text)
        self.assertIn("float ambient_strength = 0.84f;", d3d11_text)
        self.assertIn("float diffuse_wrap_bias = 0.58f;", d3d11_text)
        self.assertIn("float specular_base = 0.055f;", d3d11_text)
        self.assertIn("float specular_max = 0.52f;", d3d11_text)
        self.assertIn("float shininess_max = 152.0f;", d3d11_text)
        self.assertIn("const bool has_explicit_render_tuning", d3d11_text)
        self.assertIn("if (!has_explicit_render_tuning)", d3d11_text)
        self.assertIn('"diffuse_wrap_bias": float(getattr(settings, "diffuse_wrap_bias", 0.58) or 0.58)', Path("cdmw/ui/native_d3d11_preview_host.py").read_text(encoding="utf-8"))
    ...
    def test_d3d11_preview_wires_source_part_picking_and_context_event(self) -> None:
        ...
        dblclk_body = d3d11_text[dblclk_start:d3d11_text.index("case WM_LBUTTONUP:", dblclk_start)]
        ...
        pointer_start = d3d11_text.index("bool handle_pointer_down_or_move(")
        pointer_end = d3d11_text.index("bool handle_window_message(", pointer_start)
        pointer_body = d3d11_text[pointer_start:pointer_end]
        self.assertLess(
            pointer_body.index("if (begin_alignment_drag(wparam, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam)))"),
            pointer_body.index("begin_source_part_click(wparam, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));"),
        )
        self.assertLess(
            pointer_body.index("begin_source_part_click(wparam, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));"),
            pointer_body.index("if (begin_mesh_edit_drag(wparam, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam)))"),
        )
        interaction_start = d3d11_text.index("bool handle_interaction_commands(")
        interaction_end = d3d11_text.index("bool handle_edit_state_commands(", interaction_start)
        self.assertIn('command == "set_source_part_picking"', d3d11_text[interaction_start:interaction_end])
    ...
    def test_d3d11_preview_does_not_overpaint_duplicate_base_material_layer(self) -> None:
        ...
        self.assertIn("layer_alpha[ID] = saturate(layer_alpha[ID] * (1.0 + strong_dye_strength * 0.35));", source)
    ...
    def test_d3d11_host_does_not_use_rich_material_inputs_as_base_override(self) -> None:
        source = d3d11_preview_source()
        parse_start = source.index("static void parse_manifest_batch_material")
        parse_end = source.index("static void parse_manifest_batch_identity_and_cloth", parse_start)
    ...
    def test_d3d11_mesh_edit_mode_draws_blender_style_topology_overlay(self) -> None:
        ...
        preserve_start = source.index("bool mesh_edit_preserve_materials_for_batch")
    ...
    def test_d3d11_preview_has_first_class_emissive_slot(self) -> None:
        ...
        native_package_source = preview_core_source()
        ...
        self.assertIn('best_binding_for_role(state.bindings, mesh, "emissive"', native_package_source)
        self.assertIn('{"emissive", batch.preview_emissive}', native_package_source)
    ...
    def test_d3d11_preview_uses_procedural_reflection_for_metal_materials(self) -> None:
        ...
        self.assertNotIn("inspection_light_blend", source)
        self.assertNotIn("direct_specular = specular_brdf * key_light * saturate(raw_ndotl)", source)
    ...
    def test_d3d11_preview_caps_nonmetal_material_response_by_category(self) -> None:
        ...
        self.assertIn("if (!conservative_nonmetal)", source)
        self.assertIn("float nonmetal_specular_cap = conservative_nonmetal ? category_specular_cap : max(0.18, category_specular_cap);", source)
        self.assertIn("category_leather ? 0.14", source)
        self.assertIn("category_cloth ? 0.055", source)
        self.assertIn("category_skin ? 0.20", source)
        self.assertIn("category_skin ? 0.075", source)
        self.assertIn("category_leather ? 0.76", source)
        self.assertIn("category_cloth ? 0.84", source)
        self.assertIn("category_skin ? 0.58", source)
        self.assertIn("roughness = max(roughness, category_roughness_floor);", source)
        self.assertIn("metalness = min(metalness, category_metal_cap);", source)
        self.assertIn("specular = min(specular, category_specular_cap);", source)
        ...
        self.assertIn("material_reference_albedo", source)
        self.assertIn("stable_ao", source)
        self.assertIn("float ao_weight = saturate(render_tuning3.x) *", source)
        self.assertIn("float ambient_floor = category_metal ? 0.24", source)
        self.assertIn("float depth_authority = category_metal ? 1.00", source)
        self.assertIn("float3 color = material_reference_albedo * stable_ao * nonmetal_texture_scale * diffuse_depth * metal_diffuse_scale;", source)
        self.assertIn("float metal_cue = category_metal", source)
        self.assertIn("color += material_reference_albedo * metal_cue * 0.16;", source)
        self.assertIn("direct_metal_response", source)
        self.assertIn("env_material_scale", source)
        self.assertIn("cloth_high_luma_guard", source)
    ...
    def test_d3d11_preview_shader_uses_registry_pbr_lighting_helpers(self) -> None:
        ...
        self.assertIn("float neutral_metal_tint = early_category_metal ? saturate((0.12 - tint_chroma) * 8.0) : 0.0;", source)
