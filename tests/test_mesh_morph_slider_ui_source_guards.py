...
class MeshMorphSliderUiSourceGuardTests(unittest.TestCase):
    def test_mesh_editing_tab_exposes_morph_slider_controls(self) -> None:
        source = (ROOT / "cdmw" / "ui" / "main_window.py").read_text(encoding="utf-8")

        self.assertIn('morph_slider_title_label = QLabel("Morph Sliders")', source)
        self.assertIn('morph_slider_create_button = QPushButton("Create Slider From Selection")', source)
        self.assertIn('morph_slider_manage_button = QPushButton("Manage Profiles")', source)
        self.assertIn('morph_slider_import_action = morph_slider_manage_menu.addAction("Import Slider Pack")', source)
        self.assertIn('morph_slider_add_action = morph_slider_manage_menu.addAction("Add Slider From Target Mesh")', source)
        self.assertIn('morph_slider_reset_button = QPushButton("Reset Sliders")', source)
        self.assertIn('morph_slider_bake_button = QPushButton("Bake Sliders")', source)
        self.assertIn('mesh_edit_selection_mode_combo.addItem(label, value)', source)
        self.assertIn('("Lasso Select", "lasso")', source)
        self.assertIn('("Rectangle Select", "rectangle")', source)
        self.assertIn('mesh_edit_grow_selection_button = QPushButton("Grow Selection")', source)
        self.assertIn('mesh_edit_shrink_selection_button = QPushButton("Shrink Selection")', source)
        self.assertIn('mesh_edit_smooth_selection_button = QPushButton("Smooth / Feather Selection")', source)
    ...
    def test_morph_sliders_are_layered_and_export_through_edited_source_mesh(self) -> None:
        ...
        static_source = (ROOT / "cdmw" / "modding" / "static_mesh_replacer.py").read_text(encoding="utf-8")
