...
class AlignmentDialogSourceGuardTests(unittest.TestCase):
    ...
    def test_alignment_dialog_has_obj_mesh_edit_controls_and_revision_hooks(self) -> None:
        ...
        self.assertIn("def _mesh_edit_faces_from_payload", main_source)
        self.assertIn('group.get("source_face_indices")', main_source)
    ...
    def test_parts_outliner_has_click_edit_contract_and_copied_part_safety(self) -> None:
        ...
        self.assertIn("for raw_source_index, raw_role in tuple(source_role_overrides.items())", source)
        ...
        self.assertIn('true_source_basic_form.addWidget(QLabel("Glow color"), 8, 0)', source)
        ...
        self.assertIn("adjustment.emissive_color_rgb = _selected_part_glow_rgb_from_controls()", source)
        self.assertIn("def _apply_source_role_emissive_preview", source)
        self.assertIn("_clear_source_role_emissive_preview(mesh)", source)
        self.assertIn("if not _accent_glow_preview_enabled():", source)
        self.assertIn('confidence="source-role-preview"', source)
        self.assertIn('overrides["emissive_intensity"] = emissive_intensity', source)
        self.assertIn('overrides["emissive_color"] = color_hex', source)
        self.assertIn('reason="source glow color change"', source)
    ...
    def test_alignment_dialog_routes_visible_dds_contract_and_prune_intent(self) -> None:
        ...
        complete_swap_block = source[
            source.index("def _complete_external_swap_mappings")
            : source.index("def _apply_complete_external_swap_routing_to_ui")
        ]
        self.assertIn('raw_text = str(edit.property("committed_mapping_text") or edit.text() or "").strip()', complete_swap_block)
    ...
    def test_manual_material_profile_exposes_runtime_controls_under_combo(self) -> None:
        ...
        self.assertIn("Slider edits also queue a debounced preview refresh.", source)
        self.assertIn("Manual sliders queue preview refresh after input settles", source)
        self.assertIn("Preview refresh queued; press Apply Manual Settings to force it now.", source)
        ...
        self.assertIn("def _current_material_authority_preview_parameters", source)
        ...
        self.assertIn("material_parameters=manual_preview_parameters", source)
    ...
    def test_unsafe_material_preflight_export_override_is_loose_only(self) -> None:
        ...
        self.assertIn("from cdmw.core.material_authority_report_check import check_material_authority_report", source)
        self.assertIn("def _check_final_preview_material_authority", source)
    ...
    def test_global_gloss_reduction_slider_is_wired_to_static_options(self) -> None:
        ...
        self.assertIn("source_brightness_slider.setRange(-100, 100)", source)
        self.assertIn("source_brightness_spin.setRange(-100, 100)", source)
        self.assertIn("saved_source_brightness = max(-100, min(100, saved_source_brightness))", source)
    ...
    def test_true_source_basic_controls_are_wired_to_static_options(self) -> None:
        ...
        self.assertIn("Source brightness can dim or lift source color", source)
        ...
        self.assertIn("Adjustments updated. Preview refresh queued.", source)
        ...
        self.assertIn("Adjustments updated. Preview unchanged", source)
        self.assertIn('tuple(getattr(source_slot, "base_color_factor", ()) or ())', source)
        self.assertIn("bool(part_glow_color_checkbox.isChecked())", source)
        self.assertIn("_selected_part_glow_rgb_from_controls()", source)
        self.assertIn("source_role_rows", source)
