...
class AlignmentDialogSourceGuardTests(unittest.TestCase):
    ...
    def test_orientation_presets_are_explicit_apply_only(self) -> None:
        source = _main_window_source()
        self.assertIn('alignment_mode_combo.addItem("Auto: match original/grid flat", "auto_flat_original")', source)
        self.assertIn('alignment_mode_combo.addItem("Auto: force grid flat", "grid_flat")', source)
        self.assertIn("Force grid flat ignores original roll", source)
    ...
    def test_modify_original_clone_uses_exact_geometry_preview_and_target_highlight(self) -> None:
        ...
        self.assertIn("parts_outliner_layout.addWidget(geometry_overview_group, 0)", source)
    ...
    def test_alignment_geometry_tab_uses_compact_source_parts_master_list(self) -> None:
        source = _main_window_source()
        self.assertIn('source_parts_group = QGroupBox("Replacement Sources")', source)
        self.assertIn("source_parts_layout.addWidget(source_tree, 0)", source)
        self.assertIn("parts_outliner_layout.addWidget(source_parts_group, 0)", source)
        self.assertIn('control_tabs.addTab(parts_tab, "Parts && Routing")', source)
    ...
    def test_texture_plan_has_single_bulk_apply_action(self) -> None:
        ...
        self.assertIn('properties_group.setObjectName("MeshReplacementPropertiesInspector")', source)
        self.assertIn('MeshReplacementPropertiesIdentity', source)
        self.assertIn('MeshReplacementPropertiesAssignment', source)
        self.assertIn('MeshReplacementPropertiesDDS', source)
        self.assertIn('MeshReplacementPropertiesOutput', source)
        self.assertIn('MeshReplacementPropertiesWarnings', source)
    ...
    def test_alignment_dialog_routes_visible_dds_contract_and_prune_intent(self) -> None:
        ...
        self.assertIn('prune_unmapped_original_dds_checkbox = QCheckBox("Prune unmapped original DDS refs")', source)
        self.assertIn("prune_unmapped_original_texture_parameters=bool(prune_unmapped_original_dds_checkbox.isChecked())", source)
