...
class AlignmentDialogSourceGuardTests(unittest.TestCase):
    def test_assets_mesh_editor_tab_is_registered_as_primary_workspace(self) -> None:
        ...
        self.assertIn('self.archive_open_mesh_editor_button = QPushButton("Open in Mesh Editor...")', source)
        self.assertIn('self.archive_open_mesh_editor_button.setObjectName("ArchiveOpenMeshEditorButton")', source)
        self.assertIn("self.archive_open_mesh_editor_button.clicked.connect(self._open_current_archive_mesh_editor)", source)
        ...
        self.assertIn("self._launch_archive_mesh_editor_for_entry(current_entry)", source)
    ...
    def test_modify_original_clone_uses_exact_geometry_preview_and_target_highlight(self) -> None:
        ...
        self.assertIn('if _alignment_preview_detail_mode() == "full":\n                            return 0', source)
        self.assertIn('if total_faces >= 45_000 or _alignment_preview_detail_mode() == "fast":', source)
    ...
    def test_alignment_d3d11_preview_reuses_cached_packages_for_live_changes(self) -> None:
        ...
        self.assertIn("status event missed", source)
    ...
    def test_texture_plan_has_single_bulk_apply_action(self) -> None:
        ...
        self.assertIn("texture_transform_group.setVisible(bool(has_materials and advanced_materials_checkbox.isChecked()))", source)
    ...
    def test_modify_original_startup_defers_heavy_alignment_tables(self) -> None:
        ...
        self.assertIn("use_textures=not mesh_edit_raw_package", package_worker_block)
        self.assertIn("original_reference_material_parity=not mesh_edit_raw_package", package_worker_block)
        self.assertNotIn("original_reference_material_parity=enable_material_combiner", package_worker_block)

    def test_alignment_auto_detail_is_fast_biased_and_startup_steps_are_timed(self) -> None:
        source = _main_window_source()
        auto_start = source.index("def _alignment_auto_detail_uses_fast_package")
        auto_body = source[auto_start: source.index("def _live_alignment_preview_status_message", auto_start)]
        accurate_start = source.index("def _alignment_d3d11_uses_accurate_package")
        accurate_body = source[accurate_start: source.index("def _alignment_d3d11_package_quality", accurate_start)]
        ...
        self.assertIn("modify_original_clone_mode", auto_body)
        self.assertIn('str(label or "").strip().casefold() == "final test build preview"', auto_body)
        self.assertIn("face_count >= 80_000", auto_body)
        self.assertIn("texture_slot_count >= 4", auto_body)
        self.assertIn("return True", auto_body)
        self.assertIn("return not _alignment_auto_detail_uses_fast_package(label, model)", accurate_body)
        self.assertIn("target_total_faces=35_000", source)
        self.assertIn('if _alignment_preview_detail_mode() == "full":\n                            return 0', source)
    ...
    def test_alignment_dialog_routes_visible_dds_contract_and_prune_intent(self) -> None:
        ...
        self.assertIn('advanced_materials_checkbox = QCheckBox("Advanced Materials")', source)
        self.assertIn('advanced_materials_checkbox.setObjectName("MeshAlignmentAdvancedMaterialsCheckbox")', source)
        self.assertIn('material_route_summary_label.setObjectName("MeshAlignmentMaterialRouteSummary")', source)
        self.assertIn("def _set_advanced_materials_visible(checked: bool) -> None:", source)
        self.assertIn("manual_profile_group.setVisible(bool(manual_selected and advanced_materials_checkbox.isChecked()))", source)
        self.assertIn("texture_transform_group.setVisible(bool(has_materials and advanced_materials_checkbox.isChecked()))", source)
    ...
    def test_true_source_basic_controls_are_wired_to_static_options(self) -> None:
        ...
        self.assertIn("Preview unchanged: adjustment produced same visible textures.", source)
        ...
        self.assertIn("previous_visible_signature", queue_block)
        self.assertIn('signature.get("visible")', queue_block)
        self.assertIn("return", queue_block)
        self.assertIn("_queue_texture_preview_refresh()", queue_block)
    ...
    def test_replacement_source_selection_uses_overlay_not_preview_color_mutation(self) -> None:
        ...
        self.assertIn('static_preview_refresh_timer.start()', queue_source)
