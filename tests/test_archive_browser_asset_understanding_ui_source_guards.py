...
class ArchiveBrowserAssetUnderstandingUiSourceGuards(unittest.TestCase):
    ...
    def test_archive_file_context_menu_exposes_role_aware_actions(self) -> None:
        ...
        self.assertIn('placement_action = menu.addAction(menu_icons["family"], "Weapon Placement Studio...")', source)
    ...
    def test_archive_startup_progress_is_coalesced_for_smooth_splash(self) -> None:
        ...
        self.assertIn("progress_detail_has_count = bool(", source)
        self.assertIn("bar_format_text = f\"{percent}%\"", source)
        self.assertIn("detail_with_progress = f\"{progress_detail} [{progress_text}]\"", source)
    ...
    def test_archive_startup_defers_enhanced_and_basic_indexes_until_after_ready(self) -> None:
        source = MAIN_WINDOW.read_text(encoding="utf-8")
        self.assertIn(
            "Item-name search cache is missing or stale; archive list will open while search builds in the background.",
            source,
        )
        ...
        self.assertIn('self.archive_basic_index_state = "warming"', source)
        ...
        self.assertIn('"cache_loaded": True', source)
        ...
        self.assertIn("Item-name search cache will load after the archive list opens.", run_body)
    ...
    def test_placement_workspace_and_loose_overlay_review_are_present(self) -> None:
        ...
        self.assertIn("Build One Bulk Placement Mod Folder", source)
    ...
    def test_weapon_placement_studio_ui_and_guardrails_are_present(self) -> None:
        source = MAIN_WINDOW.read_text(encoding="utf-8")
        self.assertIn('weapon_placement_studio_action = menu.addAction(menu_icons["workflow"], "Weapon Placement Studio...")', source)
        self.assertIn("self._open_archive_weapon_placement_studio_dialog(current_entry)", source)
        ...
        self.assertIn("WEAPON_PLACEMENT_STUDIO_BODY_PREVIEW_CANDIDATES", source)
        self.assertIn('"cd_m0001_00_skeleton_nude_0001.pac"', source)
        self.assertLess(
            source.index('"cd_m0001_00_skeleton_nude_0001.pac"'),
            source.index('"cd_phm_00_nude_10_0001.pac"'),
        )
        ...
        self.assertIn("WeaponPlacementStudioPreview2D", studio_source)
        self.assertIn("2D estimate fallback", studio_source)
        ...
        self.assertIn("PAAC/ItemInfo paths are not buildable", studio_source)
        ...
        self.assertIn("ItemInfo, PAAC graphs, HKX, combat, and attack files are excluded", studio_source)
        self.assertNotIn("iteminfo.pabgb", studio_source.casefold())
        self.assertIn("no .paac graph copy", studio_source.casefold())
        ...
        self.assertIn("Legacy raw prefab copy for bulk (risky)", source)
