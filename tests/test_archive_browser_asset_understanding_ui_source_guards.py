...
class ArchiveBrowserAssetUnderstandingUiSourceGuards(unittest.TestCase):
    ...
    def test_archive_startup_builds_enhanced_indexes_before_browser_ready(self) -> None:
        source = MAIN_WINDOW.read_text(encoding="utf-8")
        ready_message = "Archive enhanced indexes are stale or missing; building before browser ready."
        self.assertIn(ready_message, source)
        ...
        self.assertNotIn("self.archive_deferred_enhanced_index_start_pending = True", source)
        ...
        self.assertIn("build_archive_item_search_index(", scan_body)
        self.assertIn("name_search_index = build_archive_name_search_index(", scan_body)
    ...
    def test_placement_workspace_and_loose_overlay_review_are_present(self) -> None:
        ...
        self.assertIn("Build Safe Placement Package", source)
        ...
        self.assertIn('visual_plan_section = QGroupBox("Visual Placement + Package Plan")', source)
        self.assertIn("section_splitter = QSplitter(Qt.Horizontal)", source)
        self.assertIn("section_splitter.setSizes([420, 560, 620])", source)
        self.assertIn("section_splitter.setStretchFactor(0, 2)", source)
        self.assertIn("section_splitter.setStretchFactor(1, 3)", source)
        self.assertIn("section_splitter.setStretchFactor(2, 3)", source)
